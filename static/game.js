...
const brickWidth = (canvas.width - 2 * 30 - (brickColumnCount - 1) * 10) / brickColumnCount; // 30px space on each side
...
function gameOver(quit) {
    ...
    ctx.fillText("Press R to restart", canvas.width / 2 - 150, canvas.height / 2 + 100);
    started = false;
    document.addEventListener("keydown", keyDownHandler, false);
    document.addEventListener("keyup", keyUpHandler, false);
}
